#!/bin/bash
# One-time import of terms.sql into the VPS PostgreSQL container.
# Run ONLY on first setup, or when you intentionally want to reset/reload data.
set -e
...
if [ ! -f "$SQL_FILE" ]; then
  ...
  echo "Upload terms.sql to $APP_DIR first, then run this script again."
  exit 1
fi
...
if [ "${TABLE_COUNT:-0}" != "0" ] && [ "${FORCE_RESTORE:-0}" != "1" ]; then
  echo ""
  echo "⚠️  Database already has $TABLE_COUNT table(s) in public schema."
  echo "Skipping restore to avoid overwriting existing data."
  echo "To force restore anyway: FORCE_RESTORE=1 bash cicd/deployment/06-restore-database.sh"
  exit 0
fi

echo "Importing $SQL_FILE (this may take a minute)..."
